...
const Mobile = ({ priceFormatter, agregarProductoAlCarrito, agregarProductoAlCarritoFav, removeItemFromCartFav, cart, favProducts, mobile }) => {
    return (
        <div className="card mr-4 mb-4 p-3 shadow p-3 mb-5 bg-white rounded " style={{ width: "200px", minWidth: "30px" }}>
            ...
            <div className="h6"> <small> Pris: {priceFormatter(mobile?.price)} </small></div>
            ...
            <div>
                ...
                {cart?.includes(mobile?.name) ? <Link to='/checkout'><button className='bg-success px-1 rounded ml-2'>Varukorgen</button></Link> : <button className="bg-warning px-1 rounded ml-2" onClick={() => agregarProductoAlCarrito(mobile?.id + 'm', mobile?.name, mobile?.price, 'mobiles', 1)}>Lägg till</button>}
            </div>
        </div>
    );
}
